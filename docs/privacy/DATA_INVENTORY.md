...
|---|---|---|---|---|
| Account identity | Email, Supabase user ID, server `app_metadata`; login and authorization | Supabase Auth; SDK session storage on device/browser | Supabase account/session controls; local session removed on sign-out | High |
| Subscription/billing | Plan/status, Stripe customer and subscription IDs, usage counts | Backend/Supabase; Stripe | Webhook/update lifecycle; no app-level retention schedule | High |
| Uploaded documents | User PDF for learning and Teacher workflows | Backend uploads, private Supabase Storage, registry | UI/cloud row deletion exists; complete file/vector cascade not yet guaranteed | High |
| Extracted text/embeddings | PDF text for search and AI context | Backend memory, OpenAI embeddings, local Chroma | No complete per-document purge proven | High |
...
| AudioBook/MP3 | Source text, chapters, learning packs, generated speech | Local cache, Supabase rows, backend MP3, OpenAI TTS | Row cleanup exists; complete MP3 lifecycle not guaranteed | High |
...
prevents A-to-B display after account switching. Data may remain on a shared
device after logout and relies on Android/browser sandboxing rather than custom
encryption. Supabase Auth token persistence is delegated to its SDK.
