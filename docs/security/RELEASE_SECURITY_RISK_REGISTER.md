...
|---|---|---|---|---|---|---|
| SEC-7D-001 | P2 | Service-role calls bypass RLS even though deployed RLS/Storage isolation passed W6. | Supabase/provider integration | Keep service role backend-only and verify Render uses the approved production project without leaking credentials. | Conditional provider gate | W6 data gate closed; Render connectivity open |
| SEC-7D-002 | P2 | Account erasure must remain consistent across Supabase Storage/rows, generated artifacts and Auth. | Privacy/deletion | W6 validated the owner-scoped retry-safe lifecycle against production; approve legal retention language and recheck from Render. | Human/legal gate | Technical data gate closed |
...
| SEC-7D-008 | P3 | `backend/.venv` was tracked despite ignore rules. | Repository hygiene | W5.1 removed 8,445 files from the current index, preserved the ignored local environment and documented recreation from requirements. | No | Closed |
...
| SEC-W5-001 | P2 | Concurrent Free requests could previously consume beyond the final monthly slot. | Cost control | PostgreSQL reservation RPC, advisory lock, idempotent usage commit, release and TTL recovery are deployed and validated. Recheck through Render after deployment. | Conditional provider gate | Remote migration gate closed |
| SEC-W7-001 | P2 | Supabase leaked-password protection remains disabled. | Authentication | Enable it in Auth password security on the production project and test signup/password changes. The feature requires Supabase Pro or above. | Yes before public launch | Human action |
| SEC-W7-002 | P2 | API rate limiting is in-memory and per Render process. | Availability/cost | Accept for controlled RC only; monitor and adopt a distributed limiter before horizontal scale. | No for controlled RC | Accepted with monitoring |
