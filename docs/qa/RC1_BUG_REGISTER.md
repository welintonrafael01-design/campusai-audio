...
| --- | --- | --- | --- | --- | --- | --- |
| RC1-P2-001 | P2 MEDIUM | Formatting | OPEN | `dart format --output=none --set-exit-if-changed .` reports 33 files would be formatted. | Baseline command returned exit code 1. No Git diff was written. | Run a dedicated formatting-only PR/commit after QA or before store build if release policy requires strict format check. |
